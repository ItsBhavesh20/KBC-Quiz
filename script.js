...
for (let i = 0; i < jsQuestions.length; i++) {
  originalquestions.push(jsQuestions[i]);
}
...
let timersound = new Audio("audio/clock.mp3");
let rightsound = new Audio("audio/correct_answer.mp3");
let wrongsound = new Audio("audio/wrong_answer.mp3");
let letsplaysound = new Audio("audio/lets_play.mp3");
let timeout = new Audio("audio/timeout.mp3");
let crore = new Audio("audio/7crore.mp3");
...
function nextquetion() {
  ...
  optiona.innerText = `${currentquestion.options.a}`;
  optionb.innerText = `${currentquestion.options.b}`;
  optionc.innerText = `${currentquestion.options.c}`;
  optiond.innerText = `${currentquestion.options.d}`;
...
let winningamount = "₹7 Crore"; // for storing winning amount
let safeamount = "0";
...
function resetgame() {
  ...
  for (let i=0;i<originalquestions.length;i++) {
    jsQuestions.push(originalquestions[i]);
  }
